...
struct thread* activeProcess[MAX_PROCESS];
int exitProcessCode[MAX_PROCESS];
struct semaphore* waitP_table[MAX_PROCESS];
int parent_table[MAX_PROCESS];

struct semaphore* arrayMutex;

// remember to initalize to NULL when boostrap
int findGap(void);
