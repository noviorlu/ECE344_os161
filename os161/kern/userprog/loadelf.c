 ...
 */
int
load_elf_origin(struct vnode *v, vaddr_t *entrypoint)
{
...
load_elf(struct vnode *v, vaddr_t *entrypoint)
{
	...
	for (i=0; i<eh.e_phnum; i++) {
		...
		// DEBUG(DB_VM, "execvFilePTR %x -- offset %x -- vaddr %x -- memsz %x -- filesz %x isExec? %d\n", (u_int32_t)v, ph.p_offset, ph.p_vaddr, ph.p_memsz, ph.p_filesz,ph.p_flags & PF_X);
		if (result) {
			return result;
		}
	}
