...
// return the number of active process
int countProcess(void){
	int counter=0;
	int i=0;
	for(i=0;i<MAX_PROCESS;i++){
		if(activeProcess[i]!=NULL){
			counter++;
		}
	}
	return counter;
...
int findGap(void){
	int i=0;
	for(i=0;i<MAX_PROCESS;i++){
		if(activeProcess[i]==NULL && parent_table[i]==-1){
			return i;
		}
	}
	for(i=0;i<MAX_PROCESS;i++){
...
thread_bootstrap(void)
{
	...
	int i=0;
	for(i=0;i<MAX_PROCESS;i++){
		activeProcess[i]=NULL;
		exitProcessCode[i] = -1;
		waitP_table[i] = sem_create("waitPi", 0);
		parent_table[i] = -1;
	}
	


	arrayMutex = sem_create("arrayMutex", 1);
	//--------------------------------------------
	    ...
	    struct thread **ret)
{
	...
	activeProcess[pid]=newguy;
	exitProcessCode[pid] = 0;
	waitP_table[pid]->count = 0;
...
int thread_join(struct thread * thread)
{
        // Replace this
        // clocksleep(5);
        P(waitP_table[thread->pid]);
		P(arrayMutex);
        V(waitP_table[thread->pid]);
		V(arrayMutex);
...
{
	P(arrayMutex);
	if(waitP_table[curthread->pid]->count == 0)	
	 ...
	 * sets spl to the highest value, disabling all interrupts
	 */
	splhigh();
