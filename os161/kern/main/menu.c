...
common_prog(int nargs, char **args)
{
	...
	// kprintf("thread:%x created safely\n", (vaddr_t)thread);
...
static
int
cmd_df(int narg, char **arg)
{
	// catch invalid arguments amount
	if(narg != 3){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	// catch invalid on/off statement
	int flag = -1;
	if(!strcmp(arg[2], "on")) flag = 1;
	if(!strcmp(arg[2], "off")) flag = 0;
	if(flag == -1){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	} 
	...
	// bit need to shift /reset all to zero
	int b = 0x1 << (num-1);
	if(flag) dbflags |= b;
	else dbflags &= ~b;
	return 0;
